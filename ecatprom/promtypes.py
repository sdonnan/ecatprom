...
class OutOfBytesError(Exception): pass
...
class Reader:
    ...
    def read_bits(self, n):
        ...
        '''
        if n > 64 or n < 1: raise ValueError('Cowardly refusing to read that many bits')
        pos = n-1
        val = 0
        while n > 0:
            if self.pos_bits <= 0:
                d = self._data.read(1)
                if len(d) != 1: raise OutOfBytesError()
    ...
    def value(self, v):
        if self.bounds:
            if v < min(self.bounds) or v > max(self.bounds):
                raise ValueError("Value {} out of bounds {}".format(v, self.bounds))
        self._value = v
...
class Enum(Int):
    ...
    def value(self, v):
        if v not in self.options.values():
            raise ValueError("Value {} not a valid enumeration {}".format(v, self.options.values()))
...
class String(Item):
    ...
    def put(self, writer):
        r = self._value
        writer.write_bytes(len(r).to_bytes(1) + r)
...
class Array(Struct):
    ...
    def __init__(self, item_type, count=None):
        self._members = []
        self._count = count
